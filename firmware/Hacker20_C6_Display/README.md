...
- **Режим USB/OFF** (`chargeOnlyMode` = кабель вставлен + тумблер разомкнут): экран питается от USB и показывает «заряд выкл», BLE-контроллер не инициализируется и Wi-Fi выключен. Изменение тумблера при подключённом USB вызывает перезапуск в нужный режим.
